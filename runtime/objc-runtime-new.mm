...
//    path 可以打印出所有加载的动态链接库
    printf("path : %s \n", path );

//    这个方法的 hasLoadMethods 用于快速判断是否有 +load 方法
    ...
    // Discover load methods
    {
        ...
        //这个方法会获取所有类的列表然后收集其中的 +load 方法，在代码里可以发现 Class 的 +load 是先执行的，然后执行 Category
        prepare_load_methods((const headerType *)mh);
    }

    // Call +load methods (without runtimeLock - re-entrant)
//    该方法内部，会创建一个 autoreleasePool 使用函数指针来动态调用类和 Category 的 +load 方法。
    call_load_methods();
}
